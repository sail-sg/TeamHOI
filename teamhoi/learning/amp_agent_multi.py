...
class AMPAgent(common_agent.CommonAgent):
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            for k in update_list:
                res_dict[k] = res_dict[k].view(self.num_actors, self.env_num_agents, -1)
        ...
        mb_amp_obs = self.experience_buffer.tensor_dict['amp_obs'] # (horizonLength, N_agents * N_envs, 1250)
        ...
        # Log Rewards
        wandb.log({"Rewards/task_rewards": torch.mean(mb_rewards_task_normalized),
                   "Rewards/amp_rewards": torch.mean(amp_rewards['disc_rewards']),
                   "Rewards/all_rewards": torch.mean(mb_rewards_normalized),
                   "epoch": self.epoch})
    ...
    def get_action_values(self, obs_dict, rand_action_probs= None):
        ...
        processed_obs = self._preproc_obs(obs_reshaped)
    ...
    def calc_gradients(self, input_dict):
        ...
        amp_obs = reshaper(input_dict['amp_obs'][0:self._amp_minibatch_size])
        amp_obs = self._preproc_amp_obs(amp_obs)
        amp_obs_replay = reshaper(input_dict['amp_obs_replay'][0:self._amp_minibatch_size])
    ...
    def _update_amp_demos(self):
        new_amp_obs_demo = self._fetch_amp_obs_demo(self._amp_batch_size) # (B, 1250)
