# ================================================================
# Tokenization + Attention (no padding)
#
...
#
# Number of agents can vary. Other-agent tokens are passed directly
# without padding, so sequence length changes with team size.
# Masking is used to ignore inactive agents when provided.
# ================================================================
...
class AMPTransformerMultiTaskBuilder(AMPBuilder):
    ...
    class UnifiedNetworkClass(AMPBuilder.Network):
        ...
        def _eval_Transformer(self, obs, mask, mask_others):
            ...
            others_token = self.task_encoder[2](others_obs)
            x = torch.cat((weight_token, self_token, obj_token, goal_token), dim=1)
            ctx = others_token
            
            if mask_others is not None:
                ff_mask = torch.ones((x.shape[0], ctx.shape[1]), dtype=torch.bool, device=x.device) # init to all True
                ff_mask[:, 0] = False
                ff_mask_others = ~mask_others[mask]
                ff_mask[..., 1:] = ff_mask_others
                x = self.transformer_encoder(x, kv=ctx,kv_key_padding_mask=ff_mask)
            else:
                x = self.transformer_encoder(x, kv=ctx)
        ...
        def _eval_Transformer_critic(self, obs, mask, mask_others):
            ...
            others_token = self.task_encoder_critic[2](others_obs) # (B, Max_agents - 1, 64)
            x = torch.cat((weight_token, self_token, obj_token, goal_token), dim=1)
            ctx = others_token
            ...
            if mask_others is not None:
                ...
                ff_mask_others = ~mask_others[mask]
                ff_mask[..., 1:] = ff_mask_others
                x = self.transformer_encoder_critic(x, kv=ctx,kv_key_padding_mask=ff_mask)
            else:
                x = self.transformer_encoder_critic(x, kv=ctx)
