...
class VecTaskPythonWrapper(VecTaskPython):
    ...
    def reset(self, env_ids=None):
        ...
        if hasattr(self.task, "mask"):
            return torch.clamp(self.task.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device), self.task.mask
        else:
    ...
    def fetch_amp_obs_demo2(self, num_samples):
        return self.task.fetch_amp_obs_demo2(num_samples)